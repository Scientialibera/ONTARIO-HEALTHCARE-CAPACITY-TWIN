...
def e2sfca_accessibility(
    ...
    for facility in facilities:
        ...
        facility_ratios[facility.id] = facility.annual_ed_capacity / weighted_demand if weighted_demand > 0 else 0.0
...
def accessibility_equity(scores: dict[str, float], populations: dict[str, int]) -> dict:
    """Population-weighted coefficient of variation of accessibility."""
    ...
    variance = sum(populations.get(key, 0) * (scores[key] - avg) ** 2 for key in scores) / total_pop
